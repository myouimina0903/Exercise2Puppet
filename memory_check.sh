...
if [ $critical_threshold -gt $warning_threshold ]
     then
     echo "Provided Value of Critical and Warning Threshold are OK"
     else
     echo "Misleading value of Inputs"
fi
...
if [[ "$result" -gt "$(($critical_threshold))" ]]
     then
     echo "Memory Usage has reached a critical level"
     echo $(sendmail $email  < email.txt)


elif [[ "$result" -gt "$warning_threshold" ]]
     then
     echo "Memory Usage has reached the Warning Threshold"
fi
